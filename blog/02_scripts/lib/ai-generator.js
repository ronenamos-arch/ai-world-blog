...
function buildPrompt({ name, prompt, urlSource, partialContent, scrapedContent, jevContext }) {
  const sources = [];
  ...
  if (jevContext) {
    sources.push(
      `ניתוח מקדים ומטרות המרה (Jev Triage Engine):
- סוג פוסט מבוקש: ${jevContext.suggestedPostType || "tool_explainer"}
- קהל יעד עיקרי: ${jevContext.targetAudience || "business_owners"}
- מיקוד CTA והנעה לפעולה: ${jevContext.recommendedCta || "course"}`
    );
  }
...
- שלב קריאה לפעולה (CTA) מותאמת לסיום המאמר`;
}
